...
public:
    ...
    // The following depend on object paths that must be provided from the sysroot interface.
    //
    ...
    // Transaction object paths are returned by methods which start a transaction on the sysroot interface: e.g. Upgrade, Deploy, etc.
    OrgProjectatomicRpmostree1TransactionInterface *transaction(QDBusObjectPath objectPath);
...
// This should get called in any class which needs access to it as a dependency.
